...
const SideBoard = () => {
  ...
  const handleSendMessage = () => {
    if (messageText.trim()) {
      // Prepare user_data JSON object
      // const user_data = {
      //   id: selectedUser.id,
      //   username: selectedUser.username,
      // };

      const data_to_socket = {
        id: loggerID,
        username: userName,
        type: userType,
      };
      ...
      if (chatElement) {
        ...
        chatElement.classList.add("flex", "items-center", "cursor-pointer");
        ...
        chatElement.innerHTML = `
          <div class="w-12 h-[45.33px] bg-gray-800 flex items-center justify-center rounded-full overflow-hidden">
            ...
            ? `<img src="${API_WEB_URL}${selectedUser.profileImg}" alt="${selectedUser.username.charAt(0)}" class="w-full h-full">`
            : `<span class="text-white text-lg flex justify-center items-center font-bold capitalize">${selectedUser.username.charAt(0)}</span>`
          }
          </div>
          <div class="ml-4 message_right">
            <div class="flex justify-between items-end">
              <div>
                <div class="text-white text-sm font-bold capitalize">${selectedUser.username}</div>
        ...
        chatElement.onclick = () => searchFollowers();
        // if (reciverType == 'master') {
        //   chatElement.onclick = () => chooseAdmin();
        // } else {
        //   chatElement.onclick = () => chooseUser(reciverType, user_data);
        // }
        const allChatList = document.querySelector(".all_chat_list");
        allChatList.prepend(chatElement);
      }
    ...
    socket.on("receive_message", (data) => {
      if (data.reciverID == loggerID && data.reciverType == userType) {
        ...
        if (chatElement) {
          ...
          chatElement.classList.add("items-center");
          chatElement.classList.add("cursor-pointer");
          chatElement.innerHTML = `
            <div class="w-12 h-[45.33px] bg-gray-800 flex items-center justify-center rounded-full overflow-hidden">
              ${data.userImg != null
              ? `<img src="${data.userImg}" alt="${data.userName.charAt(0)}" class="w-full h-full">`
              : `<span class="text-white text-lg flex justify-center items-center font-bold capitalize">${data.userName.charAt(0)}</span>`
            }
            </div>
            <div class="ml-4 message_right">
              <div class="flex justify-between items-end">
                <div>
                  <div class="text-white text-sm font-bold capitalize">${data.userName}</div>
                  <div class="text-gray-200 text-xs capitalize main_msg">${data.messageText}</div>
                </div>
                <div class="text-gray-400 text-xs uppercase msging_time">${formattedDateTime}</div>
              </div>
            </div>`;
        ...
        chatElement.onclick = () => searchFollowers();
        // chatElement.onclick = () => chooseUser(data.data_to_socket.type, data.data_to_socket);
        const allChatList = document.querySelector(".all_chat_list");
        allChatList.prepend(chatElement);
      }
    });

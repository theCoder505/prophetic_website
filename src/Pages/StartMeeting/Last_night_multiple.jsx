...
const StartMeeting = () => {
    ...
    const startStreaming = async () => {
        ...
        try {
            ...
            if (userType == 'Member') {
                ...
                uniqueKey = userType + loggerID;
            }
            // Combine userType and loggerID to create a unique key
    ...
    const createPeerConnection = async (uniqueKey, stream) => {
        try {
            console.log('Initializing peer connection for:', uniqueKey);
    
            const peerConnection = new RTCPeerConnection(configuration);
            peerConnections.current.set(uniqueKey, peerConnection);
    
            // Ensure events are set up before creating an offer
            console.log('Setting up event handlers for:', uniqueKey);
            ...
            peerConnection.ontrack = (event) => {
                ...
                if (!remoteVideoRefs.current.has(uniqueKey)) {
                    ...
                    // Store and append the video element
                    remoteVideoRefs.current.set(uniqueKey, videoElement);
                    document.getElementById('remote-videos').appendChild(videoElement);
                    console.log("Remote video element added for:", uniqueKey);
                } else {
                    console.log("Video element for this peer already exists:", uniqueKey);
                }
            };
    
            // Verify event handlers were added
            console.log('Event handlers added for peer connection:', uniqueKey);
            ...
            console.log('Offer sent to:', uniqueKey);

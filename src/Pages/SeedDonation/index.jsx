...
const SeedDonation = () => {
  ...
  const submitDonation = async (
    ...
    try {
      const response = await fetch(`${API_DOMAIN}/seed-donation`, {
        method: 'POST',
        headers: {
  ...
  // for seed donations to master admin 
  useEffect(() => {
    if (userType == 'master') {

      axios.post(
        `${API_DOMAIN}/fetch-seed-donations`,
        { userType, userEmail },
        {
          headers: {
            'Content-Type': 'application/json',
              ...
              <div className="bg-white min-h-screen h-full">
                <div className="spec_header">
                  ...
                  <div className="flex row space-x-20 pl-10 pt-2 pr-10 w-30 pb-1 font-roboto-slab">
                    <div>
                      <h6 className="text-xl text-white font-medium">Seed Donation</h6>
                    </div>
                    <div>
                      <Link to={"/tithedonations"} className="text-xl text-gray-400">Tithe Donation</Link>
                    </div>
                  </div>
                </div>
                ...
                <div className={`px-8 py-4 ${userType !== 'master' ? '' : 'hidden'} `}>
                  <div className="flex row justify-between">
                    <p className="w-40 sm:w-96 text-gray-400">
                      Contribution given by individuals to a church or
                      ministry with the belief  that it will bring  blessings,
                      prosperity, rewards from God.
                    </p>
                    <img
